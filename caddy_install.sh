...
Download_caddy(){
	...
	if [[ ${bit} == "x86_64" ]]; then
		wget --no-check-certificate -O "caddy_linux.tar.gz" "https://mirrors.salty.ml/caddy-linux-amd64.tar.gz${extension_all}"
	elif [[ ${bit} == "i386" || ${bit} == "i686" ]]; then
		wget --no-check-certificate -O "caddy_linux.tar.gz" "https://mirrors.salty.ml/caddy-linux-386.tar.gz${extension_all}"
